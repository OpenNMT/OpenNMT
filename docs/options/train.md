...
* `-sample_vocab [<boolean>]` (default: `false`)<br/>Use importance sampling approach as approximation of full softmax: target vocabulary (or bitext), or source vocabulary (for language model) is built using sample.
