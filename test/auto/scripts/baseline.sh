...
th preprocess.lua -train_src ${DATA}/${NAME}_train.${SRC}.tok -train_tgt ${DATA}/${NAME}_train.${TGT}.tok -valid_src ${DATA}/${NAME}_valid.${SRC}.tok -valid_tgt ${TMP}/${NAME}_valid.${TGT}.tok -save_data ${DATA}/${NAME} ${PARAMS_PREPROCESS} > ${TMP}/preprocess.log
if [ $? -ne 0 ]; then
  err = $?
