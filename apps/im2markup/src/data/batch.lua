...
function Batch:set_encoder(features)
    ...
    self.source_length = features:size(1)
    self.source_input_features = {}
    self.source_input_rev_features = {}
    self.source_input = features
    self.source_input_rev = self.source_input:index(1, torch.linspace(self.source_length, 1, self.source_length):long())
    return self
end
--[[ Im2Markup: set with a sequence of features
  * target_input: a tensor of size sequence_length, batch_size
--]]
function Batch:set_decoder(target_input, target_output, context_length)
    self.target_input, self.target_output = target_input, target_output
    self.source_length = context_length
    self.size = target_input:size(2)
    self.total_size = self.size
    self.target_length = target_input:size(1)
